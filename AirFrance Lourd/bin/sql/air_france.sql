...
BEGIN
    ...
    INSERT INTO Pilote (nom, prenom, email, mdp, adresse, statut, nbheuresvol)
    VALUES (p_nom, p_prenom, p_email, p_mdp, p_adresse, p_statut,p_nbheuresvol);
...
CREATE TABLE Avion (
    ...
    nomAeroport INT(3),
    PRIMARY KEY (idavion),
    FOREIGN KEY (nomAeroport) REFERENCES Aeroport(idaeroport)
);
...
CREATE VIEW VueNombreVolsParPilote AS
SELECT p.idpilote, p.nom, p.prenom, COUNT(v.idvol) AS nombre_de_vols, p.nbheuresvol as totalHeures
FROM Pilote p
LEFT JOIN Vol v ON p.idpilote = v.idpilote
GROUP BY p.idpilote, p.nom, p.prenom;

DELIMITER //
CREATE TRIGGER updateTHapresInsertVol AFTER INSERT ON Vol
FOR EACH ROW
BEGIN
    DECLARE heures INT;
    set heures = hour(timediff(new.heure_arrivee, new.heure_depart)); 
    UPDATE Pilote SET nbheuresvol = nbheuresvol + heures WHERE idpilote = new.idpilote; 
END //
DELIMITER ;

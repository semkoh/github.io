...
public class PanelPilotes extends PanelPrincipal implements ActionListener {
    ...
    private JComboBox<Integer> comboBoxIdCompagnie = new JComboBox<>();
    ...
    public PanelPilotes() {
       ...
       // this.panelForm.add(this.btEnregistrer);
        ...
        this.panelForm.add(new JLabel("ID Compagnie:"));
        this.panelForm.add(this.comboBoxIdCompagnie);
        ...
        ArrayList<Compagnie> idsCompagnies = Controleur.selectAllCompagnies("");
        for (Compagnie idCompagnie : idsCompagnies) {
            comboBoxIdCompagnie.addItem(idCompagnie.getIdcompagnie());
        }
        ...
        this.tablePilotes.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() >= 2) {
                    ...
                    txtEmail.setText(mdp);
                    txtEmail.setText(adresse);
                    txtEmail.setText(statut);
                    txtEmail.setText(nbheuresvol);
    ...
    public void actionPerformed(ActionEvent e) {
    	  if (e.getSource() == this.txtStatut) {
            ...
            int nbHeuresVol = 0; // Initialisation de nbHeuresVol à 0

            // Vérification et récupération du nombre d'heures de vol
            try {
                nbHeuresVol = Integer.parseInt(this.txtNbHeuresVol.getText());
                if (nbHeuresVol < 0 || nbHeuresVol > 10000) {
                	JOptionPane.showMessageDialog(this, "Nombre d'heures de vol doit être un entier entre 0 et 10000", "Erreur", JOptionPane.ERROR_MESSAGE);

                }
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(this, "Nombre d'heures de vol doit être un nombre entier", "Erreur", JOptionPane.ERROR_MESSAGE);
                return; 
            ...
            if (this.txtStatut.getSelectedItem().toString().equals("Externe")) {
                // Récupérer les informations spécifiques au pilote externe depuis les champs de l'interface utilisateur
                String dateDebut = this.txtDateDebut.getText();
                String dateFin = this.txtDateFin.getText();
                int idCompagnie = (Integer) this.comboBoxIdCompagnie.getSelectedItem();
                statut = "PiloteExterne";
                // Créer un pilote externe avec toutes les informations saisies
               Externe externe = new Externe(nom, prenom, email, mdp, adresse, statut, nbHeuresVol, idCompagnie,dateDebut, dateFin);

                // Insérer le pilote externe dans la base de données
                Controleur.insertExterne(externe);

                // Sélectionner le pilote inséré pour obtenir son ID
                ...
                // Mettre à jour le libellé indiquant le nombre de pilotes disponibles
                lbPilotes.setText("Nombre de pilotes disponibles : " + unTableau.getRowCount());
            }
            else {
                // pilote interne
                ...
                // Créer un pilote interne avec les informations supplémentaires
